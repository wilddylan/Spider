...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/beimii.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.beimii.com/',
             'Referer':'http://www.beimii.com/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              doc = pq(html)
              #doc = pq(url
              divs = doc('div.feed_tegao')
              for li in divs.items():
                url = li('p.picture').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                yuedu_id  = m[0]
		print url
                title = "\" "+li('h4').children('a').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('p.picture').children('a').children('img').attr('src')
                print pic
                imageUrl=qiniuUpdate(pic.strip())
                date = li('div.infos').children('span').text()
                ...
                sql = "INSERT INTO 3rd_yuedubook(id,creator,modifier,create_time,modify_time,is_deleted,yuedu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+yuedu_id+ "'," +title.strip() + ",'" + date.strip() +"','"+pic.strip()+"','0','','"+imageUrl+"','贝米','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
