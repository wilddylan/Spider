...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/163ddyuedu.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://d.news.163.com/',
             'Referer':'http://d.news.163.com/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              doc = pq(html)
              #doc = pq(url)
              divs = doc('div.takh_flow_box')
              for li in divs.items():
                url_id = li('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                url = 'http://d.news.163.com'+str(url_id)
                yuedu_id  = m[0]
		print url
                title = "\" "+li('h3').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('div.takh_flow_boxt').children('img').attr('data-original')
                print pic
                ...
                sql = "INSERT INTO 3rd_yuedubook(id,creator,modifier,create_time,modify_time,is_deleted,yuedu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+yuedu_id+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','网易哒哒','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
