...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/kexueren.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.guokr.com/scientific/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              #doc = pq(html)
              doc = pq(url)
              divs = doc('div.article')
              for li in divs.items():
                if li('h3') is None:
                    continue
                url = li('h3').children('a').attr('href')
		...
		print url
                ...
                sql = "INSERT INTO 3rd_yuedubook(id,creator,modifier,create_time,modify_time,is_deleted,yuedu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+yuedu_id+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','科学人','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
