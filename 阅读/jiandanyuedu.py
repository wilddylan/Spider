...
class MyHTMLParser(HTMLParser):
  def __init__(self):
    ...
    self.file = open("/app/yxtk/script/data/jiandan.sql",'a',buffering=-1)
    self.links = []
    self.img_url = '';
    self.wap_url = '';
    self.title = '';
  ...
  def handle_starttag(self, tag, attrs):
    if tag == "img":
      for name,val in attrs:
        if name == "class" and val == "lazy":
           for name,val in attrs:
              if name == "data-original":
                 self.img_url ='http:'+str(val);
    if tag == "a":
        for name,val in attrs:
            if name == "href":
               for name,val in attrs:
  ...
  def handle_data(self,data):
        ...
        if self.id_found and self.title_found:
             ...
             req = urllib2.Request(self.wap_url.strip(),data,headers)
             ...
             sql = "INSERT INTO 3rd_yuedubook(id,creator,modifier,create_time,modify_time,is_deleted,yuedu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','1000'," +self.title.strip() + ",'" + self.date.strip() +"','"+self.img_url.strip()+"','0','','"+imageUrl+"','煎蛋网','','"+p.strip()+"',0,NULL,0);"+'\n'
             print sql;
...
if __name__ == '__main__':  
  try:
     ...
     headers = {
                ...
                'Cookie':'aliyungf_tc=AQAAABWxHT7MEwkA0kFs2qm6ZkbohYLo; _ga=GA1.2.473500158.1457510541; Hm_lvt_fd93b7fb546adcfbcf80c4fc2b54da2c=1457510541,1457589734; Hm_lpvt_fd93b7fb546adcfbcf80c4fc2b54da2c=1457590328',
