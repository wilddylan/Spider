...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/jiandan.sql",'w',buffering=-1)
      ...
      for j in range(1,2):
          ...
          try:
              ...
              for li in divs.items():
                url = li('a').attr('href')
		print url
                title = "\" "+li('div.indexs').children('h2').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                ...
                sql = "INSERT INTO 3rd_yuedubook(id,creator,modifier,create_time,modify_time,is_deleted,yuedu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','12'," +title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','煎蛋网','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
