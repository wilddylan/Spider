...
def fetchMiaopaiData():
  ...
  f1 = open("/home/readyidu/script/data/qqgame.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://games.qq.com/',
             'Referer':'http://games.qq.com/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              #doc = pq(html)
              doc = pq(url)
              divs = doc('div.pic_txt_list')
              for li in divs.items():
                url = li('a.pic').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                game_id  = m[1]
		print url
                title = "\" "+li('h3').children('a').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.pic').children('img').attr('src')
                print pic
                imageUrl=qiniuUpdate(pic.strip())

                res = requests.get(url)
                html1 = unicode(res.content,'gb18030')
                res.close()
                html1 = re.sub(r'<\s*script[^>]*>[^<]*<\s*/\s*script\s*>','',html1)
                html1 = re.sub(r'<\s*style[^>]*>[^<]*<\s*/\s*style\s*>','',html1)
                doc1 = pq(html1)
                con = doc1('#Cnt-Main-Article-QQ')
                p = con('#Cnt-Main-Article-QQ').html()
                if p is None or p =='':
                  continue
                p = re.sub(r'<(?!img).*?>','',p)
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(game_id)+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','腾讯游戏','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
