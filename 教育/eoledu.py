...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/eoledu.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://news.eol.cn/',
             'Referer':'http://news.eol.cn/',
      ...
      for j in range(1,2):
          ...
          try:
              ...
              for li in divs.items():
                url_id = li('dt').children('a').attr('href')
                if url_id.find('http')==0:
                    continue
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                print m
                edu_id  = m[2]
                url = 'http://news.eol.cn/'+str(url_id)
		print url
                title = "\" "+li('span').children('a').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic_id = li('dt').children('a').children('img').attr('src')
                pic = 'http://news.eol.cn/'+str(pic_id)
                print pic
                ...
                p = re.sub(r'src=".','src="http://news.eol.cn/'+m[0]+'/'+m[1]+'',p)
                ...
                sql ="INSERT INTO 3rd_souhuedu(id,creator,modifier,create_time,modify_time,is_deleted,edu_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','" + edu_id +"'," +title.strip() + ",now(),'"+pic+"','0','','"+imageUrl+"','中国教育在线','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
