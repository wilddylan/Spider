...
def fetchMiaopaiData():
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          url = 'http://36kr.com/api/post?column_id=104&b_id=&per_page=20&_=1491892407271'                                                   
          try:
              ...
              for i in range(1,pageNo):
                url_id = result[i]['id'];
                url = 'http://36kr.com/p/'+str(url_id)+'.html'
                print url
                title = "\" "+result[i]['title'].encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = result[i]['cover'];
                imageUrl=qiniuUpdate(pic.strip())
                timet = result[i]['published_at'];
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys','"+timet.strip()+"','"+timet.strip()+"','n'," +title.strip() + ",'"+timet.strip()+"','"+imageUrl+"','"+imageUrl+"','36氪','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
