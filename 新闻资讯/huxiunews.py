...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/huxiunews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              res.close()
              doc = pq(html)
              divs = doc('div.mod-art')
              for div in divs.items():
                url_1 = div('div.mod-thumb').children('a').attr('href')
                if url_1 is None:
                  continue
                url_1 = 'https://www.huxiu.com'+str(url_1)
		print url_1
		pic = div('div.mod-thumb').children('a').children('img').attr('data-original')
                if pic is None:
                  continue
                print pic
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," +title.strip() + ",now(),'"+imageUrl+"','"+imageUrl+"','虎嗅网','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
