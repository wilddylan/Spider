...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/tmtnews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,9):
          time.sleep(1)
          if j == 1:
            url='http://www.tmtpost.com/column/2581216'
          if j == 2:
            url='http://www.tmtpost.com/'
          if j == 3:
            url='http://www.tmtpost.com/column/2446155'
          if j == 4:
            url='http://www.tmtpost.com/column/2446153'
          if j == 5:
            url='http://www.tmtpost.com/column/2446157'
          if j == 6:
            url='http://www.tmtpost.com/column/2573550'
          if j == 7:
            url='http://www.tmtpost.com/column/2573543'
          if j == 8:
            url='http://www.tmtpost.com/column/2573634'
          print url;                                                     
          try:
              ...
              for div in divs.items():
                url_1 = div('a.title').attr('href')
                if url_1 is None:
                  continue
                url_1 = 'http://www.tmtpost.com'+str(url_1)
		print url_1
		pic = div('div.pic').children('a').children('img').attr('src')
                print pic
                imageUrl=qiniuUpdate(pic.strip())
                title = "\" "+div('a.title').text()+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," +title.strip() + ",now(),'"+imageUrl+"','"+imageUrl+"','钛媒体','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
