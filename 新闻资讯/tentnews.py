...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/tentnews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,8):
          time.sleep(1)
          if j == 1:
            url='http://news.qq.com/top_index.shtml'
          if j == 2:
          ...
          try:
              ...
              doc = pq(url)
              texttt = doc('div#news')
              divs = doc('div.Q-tpList')
              for div in divs.items():
                url_1 = div('div.Q-tpWrap').children('a').attr('href')
                if url_1 is None:
                  continue
		print url_1
		pic = div('div.Q-tpWrap').children('a').children('img').attr('src')
		if pic is None:
                    pic = div('div.Q-tpWrap').children('a').children('img').attr('_src')
                print pic
                if pic is None:
                  continue
                imageUrl=qiniuUpdate(pic.strip())
                title = "\" "+div('em').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                ...
                html = unicode(res.content,'gbk')
                res.close()
                doc = pq(html)
                source = doc('span.a_source').children('a').text()
                con = doc('div.Cnt-Main-Article-QQ')
                con('img').removeAttr("style")
                con('img').removeAttr("width")
                con('img').removeAttr("height")
                con('img').attr("style","width:100%")
                p = con('div.Cnt-Main-Article-QQ').html()
                if p is None or p =='':
                  continue
                p = re.sub(r'&#13;','',p)
