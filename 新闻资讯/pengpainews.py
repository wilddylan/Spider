...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/pengpainews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,10):
          time.sleep(1)
          if j == 1:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25462'
          if j == 2:
            url='http://www.thepaper.cn/index_masonry.jsp?topCids=1664890'
          if j == 3:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25489'
          if j == 4:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25490'
          if j == 5:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25422'
          if j == 6:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25429'
          if j == 7:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25425'
          if j == 8:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25436'
          if j == 9:
            url='http://www.thepaper.cn/list_masonry.jsp?nodeid=25432'
          print url;                                                     
          try:
              ...
              res.close()
              doc = pq(html)
              divs = doc('div.news_li')
              for div in divs.items():
                url_1 = div('div.news_tu').children('a').attr('href')
                if url_1 is None:
                  continue
                url_1 = 'http://www.thepaper.cn/'+str(url_1)
		print url_1
		pic = div('div.news_tu').children('a').children('img').attr('src')
                if pic is None:
                  continue
                print pic
                imageUrl=qiniuUpdate(pic.strip())
                title = "\" "+div('h2').children('a').text()+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," +title.strip() + ",now(),'"+imageUrl+"','"+imageUrl+"','澎湃新闻','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
