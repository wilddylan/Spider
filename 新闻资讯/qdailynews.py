...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/qdailynews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,6):
          ...
          if j == 3:
            url='http://www.qdaily.com/categories/3.html'
          if j == 4:
            url='http://www.qdaily.com/categories/5.html'
          if j == 5:
            url='http://www.qdaily.com/categories/54.html'
          print url;                                                     
          try:
              ...
              res.close()
              doc = pq(html)
              divs = doc('div.packery-item')
              for div in divs.items():
                q_id = div('a').attr('href')
                q_url = 'http://www.qdaily.com'+str(q_id)
		print q_url
                m = re.findall(r'(\w+[0-9]+)\w*',str(q_id))
                q_id  = m[0]
                q_title = "\" "+div('h3').text().encode('utf-8')+" \""
                q_title = q_title.replace("\n",'')
                q_title = q_title.replace(",",'，')
                print q_title
                q_pic = div('div.pic').children('img').attr('data-src')
                print q_pic
                ...
                sql = "INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," +q_title.strip() + ",now(),'"+imageUrl+"','"+imageUrl+"','好奇心日报','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
