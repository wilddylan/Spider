...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/yidiannews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://www.yidianzixun.com/home/q/news_list_for_channel?channel_id=hot'
          if j == 2:
            url='http://www.yidianzixun.com/home/q/news_list_for_channel?channel_id=best'
          print url;                                                     
          try:
              ...
              for i in range(1,pageNo):
                url_1 = result[i]['itemid']
                if url_1 is None:
                  continue
                url_1 = 'http://www.yidianzixun.com/article/'+str(url_1)
		print url_1
		pic = result[i]['image']
		m = re.match('http',str(pic))
		if m is None:
                  continue
                if pic is None:
                  continue
                print pic
                imageUrl=qiniuUpdate(str(pic))
                ...
                if source is None or source =='':
                  sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','" +title.strip() + "',now(),'"+imageUrl+"','"+imageUrl+"','一点资讯','"+p.strip()+"',0,NULL,0);"+'\n'
                else:
                  sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','" +title.strip() + "',now(),'"+imageUrl+"','"+imageUrl+"','"+source.strip()+"','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
