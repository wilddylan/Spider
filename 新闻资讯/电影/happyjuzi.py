...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/happyjuzi.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,4):
          time.sleep(1)
          if j == 1:
            url='http://www.happyjuzi.com/gossip/'
          if j == 2:
            url = 'http://www.happyjuzi.com/movie/'
          if j == 3:
            url = 'http://www.happyjuzi.com/unity/'
          print url;                                                     
          try:
              encoding_support = ContentEncodingProcessor
              res = requests.get(url)
              html = unicode(res.content,'utf-8')
              res.close()
              doc = pq(html)
              #doc = pq(url)
              divs = doc('div.pc-item')
              for div in divs.items():
                url_id = div('a.pc-item-left').attr('href')
                if url_id is None:
                  continue
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                print m
                movie_id  = m[0]
                url = 'http://www.happyjuzi.com'+str(url_id)
		print url
                title = "\" "+div('h2').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," + title.strip() + ",now(),'" +imageUrl+ "','"+imageUrl+"','橘子娱乐','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
