...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/mtimemovienews.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,8):
          time.sleep(1)
          if j == 1:
            url = 'http://news.mtime.com/movie/all/index.html#nav'
          if j == 2:
            url = 'http://news.mtime.com/#nav'
          if j == 3:
            url = 'http://news.mtime.com/hot/#nav'
          if j == 4:
            url = 'http://news.mtime.com/tv/all/#nav'
          if j == 5:
            url = 'http://news.mtime.com/music/all/#nav'
          if j == 6:
            url = 'http://news.mtime.com/industry/all/#nav'
          if j == 7:
            url = 'http://news.mtime.com/people/all/#nav'
          print url
          try:
              ...
              res.close()
              doc = pq(html)
              lis = doc('div.news-cont')
              for li in lis.items():
                  movie_url = li('h4').children('a').attr('href')
                  ...
                  if li('img').attr('src') is None:
                     ...
                     movie_pic = li('img').attr('src')
                  movie_title = li('h4').children('a').text()
                  ...
                  sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," + movie_title.strip() + ",now(),'" +imageUrl+ "','"+imageUrl+"','时光网','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
