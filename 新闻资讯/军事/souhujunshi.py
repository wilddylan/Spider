...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/souhujunshi.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,7):
	      ...
	      #print html
              res.close()
              doc = pq(html)
              divs = doc('div.news-box')
              for div in divs.items():
                jun_url = div('div.pic').children('a').attr('href')
                if jun_url is None:
                  continue
		print jun_url
		url = 'http:'+str(jun_url)
                m = re.findall(r'(\w+[0-9]+)\w*',str(jun_url))
	        print m
                jun_id  = m[0]
                jun_title = "\" "+div('h4').children('a').html()+" \""
                jun_title = jun_title.replace("\n",'')
                jun_title = jun_title.replace(",",'，')
                print jun_title
                jun_pic = div('div.pic').children('a').children('img').attr('src')
                if jun_pic is None:
                    continue
                print jun_pic
                pic = 'http:'+str(jun_pic)
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," +jun_title.strip() + ",now(),'"+imageUrl+"','"+imageUrl+"','搜狐网','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
