...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/zcool.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.zcool.com.cn/',
             'Referer':'http://www.zcool.com.cn/',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://www.zcool.com.cn/works/0!0!!0!0!0!1!1!!!/'
          if j == 2:
            url='http://www.zcool.com.cn/articles/'
          print url;                                                     
          try:
              ...
              #doc = pq(url)
              lis = doc('li')
              for div in lis.items():
                url = div('a.image-link').attr('href')
                if url is None:
                  continue
		print url
                title = "\" "+div('a.image-link').children('img').attr('alt').encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = div('a.image-link').children('img').attr('src')
                print pic
                ...
                sql = "INSERT INTO 3rd_design(id,creator,modifier,create_time,modify_time,is_deleted,design_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','2000'," +title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','站酷','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
