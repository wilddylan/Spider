...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/acsdesign.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.acs.cn/',
             'Referer':'http://www.acs.cn/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              #res.close()
              doc = pq(html)
              divs = doc('ul.message-content-list')
              divs = divs('li')
              for li in divs.items():
                url = li('div.message-title-pritrue').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                desgin_id  = m[0]
		print url
                title = "\" "+li('div.works-info-title').children('strong').text()+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('div.message-title-pritrue').children('a').children('img').attr('src')
                print pic
                if pic is None:
                    continue
                ...
                sql = "INSERT INTO 3rd_design(id,creator,modifier,create_time,modify_time,is_deleted,design_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+desgin_id+"'," +title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','ACS','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
