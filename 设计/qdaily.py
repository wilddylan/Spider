...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/qdaily.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          if j == 1:
            url='http://www.qdaily.com/categories/17.html'
          #if j == 2:
		...
		print q_url
                ...
                sql = "INSERT INTO 3rd_design(id,creator,modifier,create_time,modify_time,is_deleted,design_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+q_id+"'," +q_title.strip() + ",now(),'"+q_pic.strip()+"','0','','"+imageUrl+"','好奇心日报','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
