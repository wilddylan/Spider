...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/sohufashion.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://fashion.sohu.com/fashion/index.shtml',
             'Referer':'http://fashion.sohu.com/fashion/index.shtml',
  ...
  while True:
      for j in range(1,8):
          time.sleep(1)
          if j == 1:
            url = 'http://fashion.sohu.com/fashion/index.shtml'
          if j == 2:
            url = 'http://fashion.sohu.com/luxuries/index.shtml'
          if j == 3:
            url = 'http://fashion.sohu.com/beauties/index.shtml'
          if j == 4:
            url = 'http://fashion.sohu.com/icon/index.shtml'
          if j == 5:
            url = 'http://fashion.sohu.com/hormone/index.shtml'
          if j == 6:
            url = 'http://fashion.sohu.com/men/index.shtml'
          if j == 7:
            url = 'http://fashion.sohu.com/lifestyle/index.shtml'
          print url;
          try:
              ...
              for div in divs.items():
                  url = div('h3').children('span.content-title').children('a').attr('href')
                  m = re.findall(r'(\w*[0-9]+)\w*',str(url))
                  print m
                  clo_id = str(m[1])
                  pic = div('div.content-pic').children('a').children('img').attr('src')
                  print pic
                  title = div('h3').children('span.content-title').children('a').html().encode('utf-8')
                  title = "\" "+title.replace('\"','\'')+" \""
                  title = title.replace("\n",'')
                  title = title.replace(",",'，')
                  print title
                  ...
                  sql = "INSERT INTO 3rd_clothes(id,creator,modifier,create_time,modify_time,is_deleted,clothes_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+clo_id+"'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','搜狐时尚','','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
