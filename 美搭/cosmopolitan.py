...
def fetchMiaopaiData():
  ...
  headers = {
             ...
             'Host':'http://fashion.cosmopolitan.com.cn/howto/',
             'Referer':'http://fashion.cosmopolitan.com.cn/howto/',
  ...
  while True:
      for j in range(1,8):
		...
		print url
                ...
                sql = "INSERT INTO 3rd_clothes(id,creator,modifier,create_time,modify_time,is_deleted,clothes_id,title,clothes_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+clo_id+"'," +title.strip() + ",'" + date.strip() +"','"+pic.strip()+"','0','','"+imageUrl+"','时尚COSMO','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
