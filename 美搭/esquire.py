...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/esquire.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.esquire.com.cn/',
             'Referer':'http://www.esquire.com.cn/',
  ...
  while True:
      for j in range(1,5):
          time.sleep(1)
          if j == 1:
            url='http://www.esquire.com.cn/c/fashion'
          if j == 2:
            url='http://www.esquire.com.cn/c/talk'
          if j == 3:
            url='http://www.esquire.com.cn/c/life'
          if j == 4:
            url='http://www.esquire.com.cn/c/people'
          print url;                                                     
          try:
              ...
              for div in divs.items():
                url_id = div('a.col-xs-3').attr('href')
                url = 'http://www.esquire.com.cn'+str(url_id)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                clo_id  = m[0]
		print url
                title = "\" "+div('div.cover-main').children('span.title').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = div('div.card-cover').children('img').attr('data-original')
                print pic
                imageUrl=qiniuUpdate(pic.strip())
                date = div('div.cover-content').children('div.text-right').text()
                ...
                sql = "INSERT INTO 3rd_clothes(id,creator,modifier,create_time,modify_time,is_deleted,clothes_id,title,clothes_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+clo_id+"'," +title.strip() + ",'" + date.strip() +"','"+pic.strip()+"','0','','"+imageUrl+"','时尚先生','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
