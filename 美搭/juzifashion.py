...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/juzifashion.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.happyjuzi.com',
             'Referer':'http://www.happyjuzi.com',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
              url='http://www.happyjuzi.com/fashion/'
          if j == 2:
              url='http://www.happyjuzi.com/beauty/'
          print url;                                                     
          try:
              ...
              res.close()
              doc = pq(html)
              divs = doc('div.pc-item')
              for div in divs.items():
                url_1 = div('a.pc-item-left').attr('href')
                url = 'http://www.happyjuzi.com'+str(url_1)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_1))
                clo_id  = m[0]
		print url
		print clo_id
                title = "\" "+div('a.pc-item-left').attr('title')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = div('a.pc-item-left').children('img').attr('src')
                print pic
                ...
                for ttt in nums.items():
                  nextpage = ttt('a.num').attr('data-url')
                  if nextpage is None:
                    continue
                  url = 'http://www.happyjuzi.com'+str(nextpage)
                  req = urllib2.Request(url)
                  res = urllib2.urlopen(req)
                  html1 = unicode(res.read(),'utf-8')
                  html1 = re.sub(r'<script>(.*?)</script>','',html1)
                ...
                sql = "INSERT INTO 3rd_clothes(id,creator,modifier,create_time,modify_time,is_deleted,clothes_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+clo_id+"'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','橘子娱乐','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
