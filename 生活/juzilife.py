...
def fetchMiaopaiData():
  uname = 'useragent.txt'
  f1 = open("/app/yxtk/script/data/juzilife.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.happyjuzi.com/interaction/',
             'Referer':'http://www.happyjuzi.com/interaction/',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://www.happyjuzi.com/interaction/'
          if j == 2:
            url='http://www.happyjuzi.com/life/'
          print url;                                                     
          try:
              ...
              lis = doc('div.pc-item')
              for li in lis.items():
                url_id = li('a.pc-item-left').attr('href')
                url = 'http://www.happyjuzi.com'+str(url_id)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                life_id  = m[0]
		print url
                title = "\" "+li('a.pc-item-left').attr('title')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.pc-item-left').children('img').attr('src')
                print pic
                ...
                sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+life_id+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','橘子生活','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
