...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/onlyladylife.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,5):
          time.sleep(1)
          if j == 1:
            url = 'http://lohas.onlylady.com/'
          if j == 2:
            url = 'http://astro.onlylady.com/'
          if j == 3:
            url = 'http://baby.onlylady.com/'
          if j == 4:
            url = 'http://shopping.onlylady.com/'
          print url;
          try:
              ...
              for div in divs.items():
                  ...
                  sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+clo_id+"'," +clo_title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','onlylady','','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
