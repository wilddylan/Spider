...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/meishichina.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.meishichina.com/',
             'Referer':'http://www.meishichina.com/',
  ...
  while True:
      for j in range(1,8):
          time.sleep(1)
          if j == 1:
            url='http://www.meishichina.com/Health/CommonSense/'
          if j == 2:
            url='http://www.meishichina.com/Health/Pulchritude/'
          if j == 3:
            url='http://www.meishichina.com/Health/Baby/'
          if j == 4:
            url='http://www.meishichina.com/Health/Virtue/'
          if j == 5:
            url='http://www.meishichina.com/Health/Food/'
          if j == 6:
            url='http://www.meishichina.com/Health/Nutrition/'
          if j == 7:
            url='http://www.meishichina.com/Health/JianKang/'
          print url;                                                     
          try:
              ...
              doc = pq(html)
              #doc = pq(url)
              lis = doc('div.cms_list')
              lis = lis('li')
              for li in lis.items():
                life_url = li('div.pic').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(life_url))
                life_id  = m[1]
		print url
                title = "\" "+li('div.detail').children('h4').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('div.pic').children('a').children('img').attr('data-src')
                print pic
                ...
                sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+life_id+ "'," +title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','美食天下','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
