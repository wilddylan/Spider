...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/raylilife.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://life.rayli.com.cn/',
             'Referer':'http://life.rayli.com.cn/',
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          url='http://life.rayli.com.cn/articles/'
          print url;                                                     
          try:
		...
		print url
                ...
                sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+life_id+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','瑞丽生活','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
