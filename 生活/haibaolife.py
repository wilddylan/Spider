...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/haibaolife.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://life.haibao.com/',
             'Referer':'http://life.haibao.com/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              lis = doc('li.fl')
              for li in lis.items():
                life_url = li('a.fl').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(life_url))
                life_id  = m[0]
		print url
                title = "\" "+li('div.fr').children('a').children('p').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.fl').children('img').attr('src')
                print pic
                ...
                sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+life_id+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','海报生活','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
