...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/lohaslife.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://lohas.pclady.com.cn/',
             'Referer':'http://lohas.pclady.com.cn/',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://lohas.pclady.com.cn/'
          if j == 2:
            url='http://health.pclady.com.cn/'
          print url;                                                     
          try:
              encoding_support = ContentEncodingProcessor
              req = urllib2.Request(url)
              res = urllib2.urlopen(req)
              html = unicode(res.read(),'GB18030')
              res.close()
              doc = pq(html)
              lis = doc('div.pics-txt')
              for li in lis.items():
                life_url = li('div.pics').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(life_url))
                life_id  = m[1]
		print url
                title = "\" "+li('div.txts').children('a.title').text()+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('div.pics').children('a').children('img').attr('src')
                print pic
                imageUrl=qiniuUpdate(pic.strip())

                req = urllib2.Request(life_url)
                res = urllib2.urlopen(req)
                html1 = unicode(res.read(),'GB18030')
                html1 = re.sub(r'<script>(.*?)</script>','',html1)
                res.close()
                doc1 = pq(html1)
                con = doc1('div.artText')
                con('img').removeAttr("style")
                con('img').removeAttr("width")
                con('img').removeAttr("height")
                con('img').attr("style","width:100%")
                p = con('div.artText').html()
                if p is None or p =='':
                  continue
                p = re.sub(r'&#13;','',p)
                ...
                sql = "INSERT INTO 3rd_guokulife(id,creator,modifier,create_time,modify_time,is_deleted,guoku_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+life_id+ "'," +title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','乐活生活','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
