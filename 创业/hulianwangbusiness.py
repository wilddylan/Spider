...
def fetchMiaopaiData():
  f1 = open("/app/yxtk/script/data/hlwbusiness.sql",'w',buffering=-1)
  headers = {
             ...
             'Host':'www.yixieshi.com',
             'Referer':'http://www.yixieshi.com/pd',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              doc = pq(url,headers)
              divs = doc('div.article-box')
              for li in divs.items():
                  bu_url = li('div.thumbnail').children('a').attr('href')
                  m = re.findall(r'(\w*[0-9]+)\w*',str(bu_url))
                  if len(m) == 1:
                     bu_id = str(m[0])
                  else:
                     bu_id = '0000'
                  if li('div.thumbnail').children('a').children('img').attr('src') is None:
                     bu_pic = " "
                     bu_id = '0000'
                  else:
                     bu_pic =li('div.thumbnail').children('a').children('img').attr('src')
                  bu_title = li('div.thumbnail').children('a').attr('title')
                  bu_title = "\" "+bu_title.replace('\"','\'')+" \""
                  bu_title = bu_title.replace("\n",'')
                  bu_title = bu_title.replace(",",'，')
                  bu_date = li('div.meta').children('time').text()
                  imageUrl=qiniuUpdate(bu_pic.strip())

                  doc1 = pq(bu_url,headers)
                  con = doc1('article.article-content')
                  con('img').removeAttr("style")
                  con('img').removeAttr("width")
                  con('img').removeAttr("height")
                  con('img').attr("style","width:100%")
                  p = con('article.article-content').html()
                  if p is None or p =='':
                    continue
                  p = re.sub(r'&#13;','',p)
                  ...
                  sql = "INSERT INTO 3rd_business(id,creator,modifier,create_time,modify_time,is_deleted,business_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+bu_id+"'," +bu_title.strip() + ",'" + bu_date.strip() +"','"+bu_pic.strip()+"','0','','"+imageUrl+"','互联网那些事','','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
