...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/xcar.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://drive.xcar.com.cn/',
             'Referer':'http://drive.xcar.com.cn/',
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              lis = doc('li.lists_li')
              for li in lis.items():
                  car_url = li('dd.tit').children('a').attr('href')
                  m = re.findall(r'(\w*[0-9]+)\w*',str(car_url))
                  car_id = m[1]
                  car_pic = li('dt').children('a').children('img').attr('src')
                  print car_pic
                  car_title = li('dd.tit').children('a').html().encode('utf-8')
                  ...
                  sql ="INSERT INTO 3rd_carnews(id,creator,modifier,create_time,modify_time,is_deleted,type_name,content_id,title,third_date,img_url,user_id,thumbnail_url,sort,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','资讯','" +car_id.strip() + "'," + car_title.strip() + ",now(),'" +car_pic.strip()+ "','','"+imageUrl+"','0','汽车试驾','','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
