...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/yicar.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://yiqishuo.yiche.com/',
             'Referer':'http://yiqishuo.yiche.com/',
      ...
      for j in range(1,2):
          ...
          try:
              ...
              for div in divs.items():
                  url_id = div('div.img').children('a').attr('href')
                  car_url = 'http://yiqishuo.yiche.com/'+str(url_id);
                  m = re.findall(r'(\w*[0-9]+)\w*',str(url_id))
                  car_id = str(m[0])+str('0')
                  car_pic = div('div.img').children('a').children('img').attr('src')
                  print car_pic
                  car_title = div('div.img').children('a').children('img').attr('alt').encode('utf-8')
                  print car_title
                  car_title = "\" "+car_title.replace('\"','\'')+" \""
                  car_title = car_title.replace("\n",'')
                  car_title = car_title.replace(",",'，')
                  print car_title
                  imageUrl=qiniuUpdate(car_pic.strip())
                  ...
                  sql ="INSERT INTO 3rd_carnews(id,creator,modifier,create_time,modify_time,is_deleted,type_name,content_id,title,third_date,img_url,user_id,thumbnail_url,sort,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','资讯','" +car_id.strip() + "'," + car_title.strip() + ",now(),'" +car_pic.strip()+ "','','"+imageUrl+"','0','易车','','"+p.strip()+"',0,NULL,0);"+'\n'
                  print sql
                  f1.writelines(sql)
                  file_name = urllib2.unquote(car_pic.strip()).decode('utf8').split('/')[-1]
