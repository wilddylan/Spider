...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/kejixunonly.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.kejixun.com',
             'Referer':'http://www.kejixun.com',
  ...
  while True:
      for j in range(1,7):
          ...
          if j == 2:
            url='http://www.kejixun.com/mobile/'
          if j == 3:
            url='http://www.kejixun.com/chanpin/'
          if j == 4:
            url='http://www.kejixun.com/kexue/'
          if j == 5:
            url='http://kepu.kejixun.com/kxjs/'
          if j == 6:
            url='http://www.kejixun.com/chuangye/'
          print url;                                                     
          try:
	      ...
	      #print html
              ...
              lis = doc('dl.imgtext')
              for li in lis.items():
                tech_url = li('dt').children('a').attr('href')
                if tech_url is None:
                  continue
		print tech_url
                m = re.findall(r'(\w*[0-9]+)\w*',str(tech_url))
	        print m
                tech_id  = m[1]
                tech_title = "\" "+li('dd').children('h2.f18').children('a').text().encode('utf-8')+" \""
                tech_title = tech_title.replace("\n",'')
                tech_title = tech_title.replace(",",'，')
                print tech_title
                tech_pic = li('dt').children('a').children('img').attr('src')
                print tech_pic
                imageUrl=qiniuUpdate(tech_pic.strip())
                req = urllib2.Request(tech_url)
                res = urllib2.urlopen(req)
                html1 = res.read()
                res.close()
                doc1 = pq(html1)
                ...
                sql = "INSERT INTO 3rd_technology(id,creator,modifier,create_time,modify_time,is_deleted,technology_id,title,technology_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status)VALUES(NULL,'sys','sys',now(),now(),'n','"+tech_id+ "'," +tech_title.strip() + ",now(),'"+imageUrl+"','0','','"+imageUrl+"','科技讯','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
                file_name = urllib2.unquote(tech_pic.strip()).decode('utf8').split('/')[-1]
