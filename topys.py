...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/topys.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.topys.cn/',
             'Referer':'http://www.topys.cn/',
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          url='http://www.topys.cn/index?category_id=61'
          print url;                                                     
          try:
              encoding_support = ContentEncodingProcessor
              doc = pq(url)
              divs = doc('div.flexible-box')
              for div in divs.items():
                if div('div.circle-img') is None:
                  continue
                url_id = div('div.circle-img').children('a').attr('href')
                url = 'http://www.topys.cn'+str(url_id)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                q_id  = m[0]
                title = "\" "+div('div.circle-text').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic_id = div('div.circle-img').children('a').children('img').attr('src')
                pic = 'http://www.topys.cn'+str(pic_id)
                ...
                sql = "INSERT INTO 3rd_design(id,creator,modifier,create_time,modify_time,is_deleted,design_id,title,third_date,img_url,wap_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+q_id.strip()+"'," +title.strip() + ",now(),'"+pic.strip()+"','"+url.strip()+"','0','','"+imageUrl+"','TOPYS','',NULL,0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
