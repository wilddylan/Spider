...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/huabian.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.huabian.com/',
             'Referer':'http://www.huabian.com/',
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          url='http://www.huabian.com/'
          print url;                                                     
          try:
              encoding_support = ContentEncodingProcessor
              doc = pq(url)
              divs = doc('div.news-item')
              for div in divs.items():
                url = div('h2').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                movie_id  = m[1]
		print url
                title = "\" "+div('h2').children('a').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = div('div.img').children('a').children('img').attr('src')
                print pic
                ...
                sql ="INSERT INTO 3rd_tencent_news(id,creator,modifier,create_time,modify_time,is_deleted,title,time,img_url,thumbnail_url,source,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n'," + title.strip() + ",now(),'" +imageUrl+ "','"+imageUrl+"','花边','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
