...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/indiegame.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://indienova.com/indie-game-news/',
             'Referer':'http://indienova.com/indie-game-news/',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://indienova.com/indie-game-news/'
          if j == 2:
            url='https://indienova.com/indie-game-development/'
          print url;                                                     
          try:
              ...
              res.close()
              doc = pq(html)
              divs = doc('div.article-panel')
              for li in divs.items():
                url_id = li('div.article-image').children('a').attr('href')
                m = re.findall(r'(\w+[a-z-0-9]+)\w*',str(url_id))
                url = 'http://indienova.com'+str(url_id)
                print m
                game_id  = m[1]
		print url
                title = "\" "+li('h4').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('div.article-image').children('a').children('img').attr('src')
                print pic
                if pic is None:
                    continue
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(game_id)+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','INDIE','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
