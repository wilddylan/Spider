...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/zhongguanGame.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          pageNo = 0;
          url=r'http://lib.wap.zol.com.cn/ipj/docList/?v=7.0&class_id=16&page='+str(j)+'&vs=and420&retina=1 ';
          print url                                                     
          try:
              ...
              for i in range(0,pageNo):
                ...
                if result[i].has_key('stitle'):
                    Game_title = "\" "+result[i]['stitle'].replace('\"','\'').encode('utf-8')+" \""
                    Game_title = dr.sub('',Game_title)
                    Game_title = Game_title.replace("\n",'')
                else:
                    Game_title = "";
                Game_date = result[i]['sdate'].encode('utf-8');
                if result[i].has_key('imgsrc'):
                    Game_pic = result[i]['imgsrc'].encode('utf-8');
                else:
                    Game_pic = "";
                Game_url = result[i]['url'];
                imageUrl=qiniuUpdate(Game_pic.strip())
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(Game_id)+ "'," +Game_title.strip() + ",'" + Game_date.strip() +"','"+Game_pic.strip()+"','0','','"+imageUrl+"','中关村在线','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
                file_name = urllib2.unquote(Game_pic.strip()).decode('utf8').split('/')[-1]
