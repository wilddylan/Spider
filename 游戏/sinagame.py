...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/sinagame.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://games.sina.com.cn/',
             'Referer':'http://games.sina.com.cn/',
  ...
  while True:
      for j in range(1,5):
          ...
          if j == 3:
            url = 'http://games.sina.com.cn/pc/'
          if j == 4:
            url = 'http://games.sina.com.cn/web/'
          print url;                                                     
          try:
              ...
              doc = pq(html)
              #doc = pq(url)
              lis = doc('div.listboxwp')
              for li in lis.items():
                img = li('img').attr('data-src')
                print img
                wap = li('h3').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(wap))
                game_id  = m[3]
                title = "\" "+li('h3').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                imageUrl=qiniuUpdate(img.strip())
                
                res = requests.get(wap)
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,gamenews_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+game_id+ "'," +title.strip() + ",now(),'"+img.strip()+"','0','','"+imageUrl+"','新浪游戏','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
                file_name = urllib2.unquote(img.strip()).decode('utf8').split('/')[-1]
