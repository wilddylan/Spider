...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/163game.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://play.163.com/',
             'Referer':'http://play.163.com/',
  ...
  while True:
      for j in range(1,4):
          ...
          if j == 2:
            url='http://play.163.com/shouyou/'
          if j == 3:
            url='http://gg.163.com/special/ggindex/'
          print url;                                                     
          try:
              ...
              #doc = pq(html)
              doc = pq(url)
              divs = doc('div.m-collist')
              for li in divs.items():
                url = li('div.m-collist').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                game_id  = m[4]
		print url
                title = "\" "+li('p.t').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('dt.img').children('img').attr('src')
                print pic
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,gamenews_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(game_id)+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','网易游戏','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
