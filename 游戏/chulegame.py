...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/chulegameonly.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.chuapp.com/',
             'Referer':'http://www.chuapp.com/',
  ...
  while True:
      for j in range(1,6):
          ...
          if j == 2:
            url='http://www.chuapp.com/category/daily'
          if j == 3:
            url='http://www.chuapp.com/category/pcz'
          if j == 4:
            url='http://www.chuapp.com/category/sd'
          if j == 5:
            url='http://www.chuapp.com/category/zsyx'
          print url;                                                     
          try:
              ...
              #html = res.content.encode('utf-8')
              #res.close()
              doc = pq(html)
              divs = doc('a.fn-clear')
              for li in divs.items():
                url_id = li('a.fn-clear').attr('href')
                url = 'http://www.chuapp.com'+str(url_id)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                game_id  = m[0]
		print url
                title = "\" "+li('a.fn-clear').attr('title')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.fn-clear').children('img').attr('src')
                print pic
                if pic is None:
                    continue
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,gamenews_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(game_id)+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','触乐','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
