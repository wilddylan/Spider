...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/joymegame.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.joyme.com/news/#areaid=1010',
             'Referer':'http://www.joyme.com/news/#areaid=1010',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://www.joyme.com/news/#areaid=1010'
          if j == 2:
            url='http://www.joyme.com/xinwen/'
          print url;                                                     
          try:
              ...
              for li in divs.items():
                url = li('dt').children('a').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                game_id  = m[1]
		print url
                title = "\" "+li('dt').children('a').children('img').attr('alt')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('dt').children('a').children('img').attr('src')
                print pic
                if pic is None:
                    continue
                ...
                sql ="INSERT INTO 3rd_zhongguangame(id,creator,modifier,create_time,modify_time,is_deleted,gamenews_id,title,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(game_id)+ "'," +title.strip() + ",now(),'"+pic.strip()+"','0','','"+imageUrl+"','着迷','','"+p.strip()+"',0,NULL,0);"+'\n'
                print sql
                f1.writelines(sql)
