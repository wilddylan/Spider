...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/tuniu.sql",'w',buffering=-1)
  ...
  while True:
      for j in range(1,2):
          ...
          try:
              ...
              lis = lis('li')
              for li in lis.items():
                url = li('a.list-img').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url))
                q_id  = m[0]
		print url
                title = "\" "+li('div.list-name').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.list-img').children('img').attr('src')
                print pic
                ...
                p = re.sub(r'src','aaa',p)
                p = re.sub(r'data-aaa',' src',p)
                p = re.sub(r'\r','',p)
                p = re.sub(r'\n','',p)
                p = re.sub(r'\s','',p)
                p = re.sub(r'aaa=',' aaa=',p)
                ...
                sql ="INSERT INTO 3rd_chanyouji(id,creator,modifier,create_time,modify_time,is_deleted,trip_id,title,trip_days,pic_count,like_count,view_count,comment_count,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status,is_top,url) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(q_id)+ "'," +title.strip() + ",'0','0','0','0','0',now(),'"+imageUrl+"','0','','"+imageUrl+"','途牛','','"+p.strip()+"',0,NULL,0,0,'"+url.strip()+"');"+'\n'
                print sql
                f1.writelines(sql)
