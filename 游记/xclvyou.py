...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/xclvyou.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://you.ctrip.com/travels/',
             'Referer':'http://you.ctrip.com/travels/',
  ...
  while True:
      for j in range(1,3):
          time.sleep(1)
          if j == 1:
            url='http://you.ctrip.com/travels/fall/t1.html'
          if j == 2:
            url='http://you.ctrip.com/travels/fall/t100.html'
          print url;                                                     
          try:
              ...
              for li in divs.items():
                url_id = li('a.city-image').attr('href')
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                trip_id  = m[1]
                url = 'http://you.ctrip.com'+str(url_id)
                title = "\" "+li('a.cpt').text().encode('utf8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('a.city-image').children('img').attr('src')
                imageUrl=qiniuUpdate(pic.strip())
                date = li('p.author').children('i').text()
                ...
                sql ="INSERT INTO 3rd_chanyouji(id,creator,modifier,create_time,modify_time,is_deleted,trip_id,title,trip_days,pic_count,like_count,view_count,comment_count,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status,is_top,url) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(trip_id)+ "'," +title.strip() + ",'0','0','0','0','0','"+date.strip()+"','"+imageUrl+"','0','','"+imageUrl+"','携程旅游','','"+p.strip()+"',0,NULL,0,0,'"+url.strip()+"');"+'\n'
                print sql
                f1.writelines(sql)
