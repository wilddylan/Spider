...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/16fantrip.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.16fan.com/',
             'Referer':'http://www.16fan.com/',
  ...
  while True:
      for j in range(1,2):
		...
		print url
                ...
                title = title.replace(",",'，')
                print title
                pic = li('a.m_item_img').children('img').attr('data-original')
                print pic
                ...
                sql ="INSERT INTO 3rd_chanyouji(id,creator,modifier,create_time,modify_time,is_deleted,trip_id,title,trip_days,pic_count,like_count,view_count,comment_count,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status,is_top,url) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(q_id)+ "'," +title.strip() + ",'0','0','0','0','0',now(),'"+imageUrl+"','0','','"+imageUrl+"','16番','','"+p.strip()+"',0,NULL,0,0,'"+url.strip()+"');"+'\n'
                print sql
                f1.writelines(sql)
