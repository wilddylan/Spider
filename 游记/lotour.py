...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/lotour.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://www.lotour.com',
             'Referer':'http://www.lotour.com',
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          #pageNo = 0;
          url='http://www.lotour.com/zhengwen/'
          print url;                                                     
          try:
	      ...
	      #print html
              ...
              for div in divs.items():
                trip_url = div('div.imgwrap').children('a').attr('href')
		print trip_url
                m = re.findall(r'(\w*[0-9]+)\w*',str(trip_url))
	        print m
                trip_id  = m[1]
                #trip_title = "\" "+div('div.imgwrap').children('a').children('p').text()+" \""
                #trip_title = trip_title.replace("\n",'')
                #trip_title = trip_title.replace(",",'，')
                #print trip_title
                trip_pic = div('div.imgwrap').children('ul.tag_list').children('li.fm').attr('pic')
                print trip_pic
                ...
                res.close()
                doc1 = pq(html1)
                con2 = doc1('h1.ia-title')
                trip_title = con2('h1.ia-title').text()
                con = doc1('div.text-height')
                con('img').removeAttr("style")
                con('img').removeAttr("width")
                con('img').removeAttr("height")
                con('img').attr("style","width:100%")
                p = con('div.text-height').html()
                if p is None or p =='':
                  continue
                p = re.sub(r'&#13;','',p)
                ...
                p = re.sub(r'original','src',p)
                p = re.sub(r'\r','',p)
                p = re.sub(r'\n','',p)
                p = re.sub(r'\s','',p)
                p = re.sub(r'src=',' src=',p)
                p = re.sub(r'alt',' alt',p)
                p = re.sub(r'oooo',' oooo',p)
                ...
                sql ="INSERT INTO 3rd_chanyouji(id,creator,modifier,create_time,modify_time,is_deleted,trip_id,title,trip_days,pic_count,like_count,view_count,comment_count,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status,is_top,url) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(trip_id)+ "','" +trip_title.strip() + "','0','0','0','0','0',now(),'"+trip_pic.strip()+"','0','','"+imageUrl+"','乐途','','"+p.strip()+"',0,NULL,0,0,'"+trip_url.strip()+"');"+'\n'
                print sql
                f1.writelines(sql)
