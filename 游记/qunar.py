...
def fetchMiaopaiData():
  ...
  f1 = open("/app/yxtk/script/data/qunar.sql",'w',buffering=-1)
  ...
  headers = {
             ...
             'Host':'http://travel.qunar.com/',
             'Referer':'http://travel.qunar.com/',
  ...
  while True:
      for j in range(1,2):
          time.sleep(1)
          url='http://travel.qunar.com/?from=header'
          print url;                                                     
          try:
              ...
              lis = doc('li.note_li')
              for li in lis.items():
                url_id = li('li.imgli').children('a').attr('href')
                url = 'http://travel.qunar.com'+str(url_id)
                m = re.findall(r'(\w+[0-9]+)\w*',str(url_id))
                q_id  = m[0]
		print url
                title = "\" "+li('h2.lf').children('a').text().encode('utf-8')+" \""
                title = title.replace("\n",'')
                title = title.replace(",",'，')
                print title
                pic = li('li.imgli').children('a').children('img').attr('data-original')
                print pic
                ...
                p = re.sub(r'data-idx',' data-idx',p)
                ...
                sql ="INSERT INTO 3rd_chanyouji(id,creator,modifier,create_time,modify_time,is_deleted,trip_id,title,trip_days,pic_count,like_count,view_count,comment_count,third_date,img_url,sort,user_id,thumbnail_url,source,tag,content,push_flag,recommend_flag,view_status,is_top,url) VALUES(NULL,'sys','sys',now(),now(),'n','"+str(q_id)+ "'," +title.strip() + ",'0','0','0','0','0',now(),'"+pic.strip()+"','0','','"+imageUrl+"','去哪儿','','"+p.strip()+"',0,NULL,0,0,'"+url.strip()+"');"+'\n'
                print sql
                f1.writelines(sql)
